...
class Dataset_train(Dataset):
    def __init__(self, records, aug_on, n_classes, aug_hparams):
        ...
        # TODO: add patient's list for adversarial head
        self.patients = np.array([i.split('/')[0] for i in records])
        self.patients_unique = np.unique(self.patients)
    ...
    def load_data(self, id):
        ...
        X = np.load(DATA_PATH + self.records_list[id]).astype(np.float32)
        y = json.load(open(DATA_PATH + self.records_list[id][:-4] + '.json'))
        y = np.array(y['label'])  # [0]

        X = self.preprocessing.run(X=X, zero_label=not bool(y[1]))

        # y = np.eye(self.n_classes)[y]

        # second head
        subset = self.records_list.copy()
        subset.remove(self.records_list[id])
        X_s = np.load(DATA_PATH + np.random.choice(subset)).astype(np.float32)
        X_s = self.preprocessing.run(X_s)
        y_s = [0]
