...
class Wavenet(Wavenet_pre_trained):
    def __init__(
        ...
        self.out_layer_2 = nn.Linear(out_shape, out_shape)
        self.out_layer_3 = nn.Linear(out_shape, hparams['emb_dim'])
    ...
    def forward(self, x):
        ...
        emb, _ = self.__encoder_block(x)

        emb = emb.mean(dim=2)

        # projection network
        emb = torch.relu(self.out_layer_1(emb))
        emb = torch.relu(self.out_layer_2(emb))
        emb = self.out_layer_2(emb)
