...
# model
from models.vgg_pre_trained.structure import VGG_cnn
...
class Model:
    ...
    def __init__(self, hparams, gpu=None, inference=False):
        ...
        self.logger.debug_logger.info(f'__setup_model_{time()}')

        # define model parameters
        self.__setup_model_hparams()
        self.logger.debug_logger.info(f'__setup_model_hparams_{time()}')
    ...
    def fit(self, train, valid):
        ...
        self.logger.debug_logger.info(f'start_training_{self.start_training}')
        ...
        self.logger.debug_logger.info(f'setup dataloaders_{time()}')
        # TODO: add logs
    ...
    def __setup_model(self, inference, gpu):
        ...
        if inference or gpu is None:
            ...
            self.model = VGG_cnn(hparams=self.hparams['model']).to(self.device)
        else:
            if torch.cuda.device_count() > 1:
                if len(gpu) > 1:
                    ...
                    self.model = VGG_cnn(hparams=self.hparams['model']).to(self.device)
                    self.model = DDP(self.model, device_ids=gpu)  # , output_device=gpu[0])
                else:
